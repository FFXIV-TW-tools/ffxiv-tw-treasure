...
- **檔案 ≤ 500 行（新檔）/ 遇授權牆不靜默跳過**：目前各檔遠低於門檻（最大 `app.js` 454 行），維持職責清楚。
...
> 測試基線 **3 套全綠 · 54 assert 呼叫**（core 11 / drift 5 / worker 38；`npm test` exit 0；**只准升不准降**；2026-07-11 實測）。
...
node worker/tests/worker.test.mjs  # 房間 applyOp/validate/originAllowed（含「並發加點不互蓋」證明）
