...
## 2026-08-16（同日續二）— 文案去內部術語、九座寶物庫全補齊

- **文案不再寫「解包」**（Owner）：畫面上直接講玩家要知道的事。
  ⚠️ 但**誠實性沒有跟著刪掉**，只是改寫成玩家語言：「依解包的採集等級門檻推導（解包沒有
  哪個點掉哪張圖的表）」→「符合這張圖採集等級的採集點（**不保證每個點都會出**）」。
  `names-authority` 新增一條：`t('…')` 內不得出現「解包」，同時原本兩條（來源標註、
  未收錄提示）照舊守著——去術語不能變成把限制一起藏掉。
- **九座寶物庫全補齊，品項再增**：併入 `DungeonDrop`（舊寶物庫有一批只記在那裡）——
  水城寶物庫 100→103、運河寶物庫 35→47、神殿 41→43、深層 34→45、夢羽寶境 9→16、寶殿 11→14。
  合計 264 → **302 筆**。新的三座（驚奇百寶城／育體寶殿／加加財富天坑）沒有這張表的資料。
- ⭐ **人工對照表與 Owner 提供的寶物庫列表逐條複核，九座全數吻合**（版本／等級／對應藏寶圖）。
  多學到一條：7.3 的「巡夢金庫」對應 G18——但台服 client 兩者都還沒收錄
  （`tc_ContentFinderCondition` 查無此名），拿不到 CFC id，等台服開放再補一行。
- 十三個等級全部都有掉落資訊：八人圖有寶物庫＋寶箱，單人圖只有寶箱（遊戲上就沒有寶物庫，
  不是資料缺）。逐級實測過一輪。
- 測試基線 226 → **227**。

## 2026-08-16（同日續）— 掉落改用副本解包表、採集點標籤、查價連動

Owner 指出「已知 2 項」太少，並給了灰機的加加財富天坑頁當參照。

- ⭐ **掉落物換資料源：藏寶迷宮寶箱表**（本地解包 `DungeonChest`／`DungeonChestItem`）。
  加加財富天坑 **2 → 18 項**，含**掉落機率與數量區間**；九座迷宮合計 264 筆。
  ⚠️ 灰機 wiki **不能程式化抓**（monorepo 鐵則，全路徑 CF 403）——但那頁指出的方向是對的：
  它講的是「藏寶迷宮」，而迷宮寶箱是解包裡有的東西。線索照收，資料走本地權威源。
  ⚠️ 「圖等級 → 迷宮」的對照**解包裡沒有**（`TreasureHuntRank`→`EventItem`→`InstanceContent`→`CFC`
  整條都斷開）⇒ `DUNGEON_CATALOG` 是人工對照，防呆＝**patch 閘**（掉落物 patch 必須落在該圖版本之後），
  接錯世代會當場失敗而不是靜默顯示錯的清單。
- ⭐ **名稱過濾放寬 `dump` → `dump` ＋ `dt`**：查證發現 `dt` 的來源就是 `tclocal_Item.csv`
  （台服 client 本地解包），只是 `tc_Item.csv` 那份較舊、7.x 物品多半是空字串。
  只收 `dump` 把台服真的有官方名的物品擋掉了一半（天坑 35 項只剩 18）。
  `dt` 仍須與 tclocal **逐字相同**才收；`opencc`／`tnze` 一律不收。哨兵權威源同步擴為兩份聯集。
  被擋掉的仍會在畫面上講出來（「另有 17 項台服尚未收錄官方名稱」）——默默少列就是零回饋訊號。
- **採集點標籤**：每個 pin 直接寫 `Lv.100 (13.3, 10.9)`。原本只有 hover title，手機完全看不到，
  而「要去哪個座標」正是這張圖唯一要回答的事。
- **掉落物連動查價**：每項是連到 marketboard `#/item/{id}` 的連結（跨工具 deep link 既有慣例）。
- `renderLoot` 從 app.js 拆成 `js/loot-panel.js`（app.js 441 → 408 行，兩個補充區塊各自成模組）。
- 測試基線 209 → **226**（names-authority 26→40、i18n 79→82）。
- ⚠️ 自己踩的坑記一筆：用 Python 覆寫檔案時把 LF 轉成了 CRLF，`names-authority` 的
  區塊切割 regex（`:\n`）當場對不上，報的卻是「地板守衛裡沒有 exit」——**訊息指向完全錯誤的方向**。
  改檔一律保 LF（`newline=''`），或直接用編輯工具。

## 2026-08-16 — 等級卡加「怎麼挖到＋在哪幾張圖」，第二步加寶箱掉落速查

Owner 要的三件事：地名別再折行、每張圖直接看得到採集門檻與區域、掉落物整理出來。

- **地圖卡加寬 160→220px**：最長繁中地名「庫爾札斯西部高地」(8 字) ＋「N 點」原本會折成兩行，
  而且被擠掉的是點數（比地名折行更難讀）。點數另加 `flex: 0 0 auto` 不參與壓縮。
- **等級卡新增「⛏ 採集 Lv.{lv} 以上的點可能挖到」＋挖掘區域清單**：不點進去就看得到這張圖出現在哪幾張地圖。
  ⭐ 採集等級是**解包權威值**（`GatheringItem.GatheringItemLevel`，G17=100），不是社群數字。
  ⭐ 順帶把一條會被反覆重查的事情釘進 AGENTS.md：**「哪個採集點會掉藏寶圖」在解包裡不存在**——
  掃過 `GatheringPointBase` 全 1425 列與 Teamcraft `nodes.json` 的 `items`／`hiddenItems`，
  13 張圖零命中 ⇒ 它是採集時隨機額外取得。所以只給等級門檻，不自創地點清單。
- **第二步（選地圖）新增「這張圖可能開出」**：`data/loot.json`（`tools/build-data.py` 由 Teamcraft
  `loot-sources.json` 反查，270 筆）。**延後載入**——選了等級才抓，第一步就離開的人不付這 10 KB。
  ⚠️ 這份清單是**社群整理、已知不完整**（G16 26 筆、G6 32 筆，但 G17 只有 2 筆、綠圖 0 筆），
  所以來源與「可能不完整」直接寫在畫面上，並由 `names-authority` 機械守它不會被誰順手拿掉。
  物品**名字**仍走台服解包（`name_tc_source='dump'`；1 筆国服機轉名因此被擋在外面）。
- **同一輪追加：採集點位真的畫出來**（`js/gather-map.js` ＋ `data/gather.json`，300 個點／31 張地圖；來源＝monorepo item_dict 的 lspl 目錄下 nodes.json）。
  第二步新增「去哪採到這張圖（採集 Lv.100，44 處）」，列出地區（點多的排前面），
  點地區開 modal 看該圖的點位，沿用 `app-modal` 的 `mapView`（不另寫 modal）。
  ⭐ 標記用**遊戲原生圖示**（xivapi 060438 採掘／060437 碎石／060433 採伐／060432 割草，權威表＝marketboard `NODE_TYPE_ICON`）。初版用 emoji 被 Owner 退回——AGENTS 的傳送點那條（「emoji 在米色地圖上幾乎看不到」）早就寫過同一件事，這次踩的是同一個坑，已把「地圖標記不用 emoji」獨立成一條鐵則。換成原生圖示後又發現**光有圖示還不夠**：白色線稿貼在米色地圖上一樣看不見（圖片有載入、定位也對，畫面上就是找不到）⇒ 視覺處理也照抄 marketboard `.map-pin-img`：30px ＋ 青色光暈 ＋ 黑色投影。
  ⚠️ 這是**依等級門檻推導**（取 level 恰好等於門檻的採集點），畫面上明講——解包沒有「哪個點掉哪張圖」的表。
  ⚠️ 上游 nodes 有一批節點 `map == 0`（丟掉 197 個）：不知道在哪張圖就畫不出來，硬畫會標到錯位置
  而畫面完全正常 ⇒ 顯示的點數是**下限**。
- 多語：`build-i18n-names.py` 一併 join 掉落物名與採集點地圖名（en／ja 各 242 個遊戲名詞，全數命中）。
  採集點地圖**比藏寶圖區大**（「活著的記憶」沒有挖掘點卻有 Lv.100 採集點）⇒ 生成器不能只看 `maps.json`。
- ⭐ 順手修掉一個會反覆咬人的測試結構問題：`drift.test.mjs` 的死 CSS 閘原本**逐檔列舉** JS 來源，
  新增 `gather-map.js` 當場誤報 4 個新 class 是死碼。改成掃**整個 `js/` 目錄**——
  「新增檔案要記得回來補一行」這種靠紀律的設計，漏補時給的是假紅燈。
- 測試基線 186 → **209**（names-authority 20→26，i18n 62→79＝共用哨兵演進＋新檔進掃描清單）。

## 2026-08-13 — i18n 第二站（EN／JA），並在共用層修掉四個「註解被當成程式碼」的缺陷

B-116 pilot（faux-hollows）之後的第二站。**每站真正的成本只有「字典 ＋ 一顆鈕」**——
哨兵與樣板都走 portal 共用版，本站只留 `i18n.config.json` 與一支 12 行的薄 wrapper。

做了什麼：

- **UI 軌**：126 條 key（HTML `data-i18n` ＋ 5 支 JS 的 `t()`），en／ja 各一份字典。
  字串串接一律改成**整句一條 key ＋ 插值**（`'第 {no} 點 · X:{x} Y:{y}'`）——
  片段串接在英日文的語序下組不回通順句子，這是本輪改寫幅度的主要來源。
- **資料軌**：13 個地圖名 ＋ 40 個地名，由 `tools/build-i18n-names.py` 從**官方解包 join**
  出 en／ja（地圖名走 `{en,ja}_Item.csv`，地名走 xivapi `csv/{en,ja}/PlaceName.csv`），
  寫進字典的標記區塊。**53/53 全部 join 成功**。
  ⭐ **產出刻意寫進字典而不是資料檔**：字典只在使用者真的切外語時才載入 ⇒ 繁中訪客
  （絕大多數）一個位元組都不用付。塞進 `maps.json` 就是所有人下載三份名字，直踩
  monorepo 鐵則「資料只載當前這份會用到的」。實測整份多語名詞約 1.5 KB，
  另開檔分層反而多一次請求、更貴。
- 順手修掉一句**正名後就變錯的文案**：「依生物皮命名」——台服官方名是編號式。

⭐ **接第二站的真正價值：暴露了四個共用層缺陷，全部是「註解被當成程式碼」**。
每一個在 pilot 上都看不到，因為 pilot 剛好無害：

1. `t('…')` **寫在註解裡**會被當成真 key ——共用啟動樣板本身就這麼示範。
   pilot 的示範字串剛好是它字典裡的真 key ⇒ 永遠對得到；換一個字串就爆。
2. `data-i18n-html="…"` 寫在 **HTML 註解**裡同理。
3. shim 位置檢查用 `indexOf('i18n.js')`，命中的是**說明註解**裡那四個字
   ⇒ 報「shim 在載入之後」這種與事實相反的失敗，而修法會變成「把註解刪掉」。
4. `data-i18n-attr` 無條件 `split(',')`，而 key ＝繁中原文
   ⇒ **屬性文字自己含半形逗號就被切斷**（本站的巨集說明 `( 21.0 , 14.0 )`），
   該屬性從此不翻譯，不 throw、不 log、畫面上只是少翻一句。

另補一項共用能力：**字典裡的生成區塊豁免「死 key」檢查**。遊戲名詞是 `t(變數)` 取用的，
靜態掃描天生看不到 ⇒ 53 條全被判死 key。這不是特例是通例（mit-planner 618、crafter 12,011），
故由產生器標記區塊、只豁免「誰引用了它」，**缺譯與 en/ja 對等仍照驗**。

驗證：Chrome 實測三語。EN／JA 逐頁走過三步流程（等級卡／地圖清單／挖掘點標題／統計行）
全部外語且**地名走官方解包**（Heritage Found／オルコ・パチャ）；繁中路徑實測
**`i18n.js` 一支都沒載入**（`document.scripts` 內 0 筆）＝設計意圖成立；
繁中路徑的語言鈕 fallback 接線按下去確實會記住並重載。
英文模式主內容區掃描**零殘留中文**（只剩語言鈕自稱的「繁中／日本語」，刻意保留）。
⚠️ 掃描抓到一條漏網：「綠圖」是社群慣用分級名（不是官方物品名，故不在生成區塊裡），
在英日畫面上是孤零零一張中文卡；已補手譯，且改成**靜態** `t('綠圖')` 讓漂移哨兵管得到。

測試基線 4 套 → **6 套**（新增 `names-authority` 20、`i18n` 62）。
