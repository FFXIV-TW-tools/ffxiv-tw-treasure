...
- Phase 對照（superpowers 已於 2026-07-17 退役，勿再引用其 skill）：Brainstorm／Plan→DEVLOOP 模板 `~/.claude/process/templates/`（需 spec/plan 時：spec 存 `docs/specs/<cycle>-design.md`＋front-matter、plan 存 `docs/plans/<cycle>-plan.md`；本 repo 兩目錄皆尚未建，建立時照此契約）／Build→TDD（`karpathy-guidelines`）／Verify→AGENTS.md「VERIFY」段／Review→`/code-review`（難回頭 commit 加 `adversarial-review`）。
