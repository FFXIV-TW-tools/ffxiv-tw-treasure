  ...
  async fetch(req, env) {
    try {
      ...
      if (parts.length === 1 && req.method === "POST") {
        if (!originAllowed(req)) return json({ error: "forbidden_origin" }, 403, req);   // 比照 WS 升級路徑：只放行本工具站/localhost 建房
...
export class Room {
  ...
  async fetch(req) {
    if (req.headers.get("Upgrade") === "websocket") {
      ...
      if (roomFull(this.ctx.getWebSockets().length)) return new Response("room_full", { status: 503 });   // 單房連線軟上限，擋洪水
      const st = await this.ctx.storage.get("state");
